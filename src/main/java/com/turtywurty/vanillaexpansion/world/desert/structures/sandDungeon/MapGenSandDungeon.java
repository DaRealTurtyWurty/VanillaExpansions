package com.turtywurty.vanillaexpansion.world.desert.structures.sandDungeon;
...
public class MapGenSandDungeon extends MapGenStructure
{
	public final List<Biome> allowedBiomes;
	private boolean ranBiomeCheck;
	private ChunkPos[] dunegonCoords;
	private double distance;
	private int spread;
	private final Biome biome;
	
	public MapGenSandDungeon() 
	{
		this.dunegonCoords = new ChunkPos[128];
		this.distance = 32.0D;
		this.spread = 3;
		this.allowedBiomes = Lists.<Biome>newArrayList();
		this.biome = new BiomeNewDesert();
		
		if(biome != BiomeInit.NEW_DESERT && biome != null)
		{
	...
	public MapGenSandDungeon(Map<String, String> name)
	{
		this();
		for(Entry<String, String> entry : name.entrySet())
		{
			...
			{
				this.dunegonCoords = new ChunkPos[MathHelper.getInt(entry.getValue(), this.dunegonCoords.length, 1)];
			}
	...
	public BlockPos getNearestStructurePos(World worldIn, BlockPos pos, boolean findUnexplored) 
	{
		...
		for(ChunkPos chunkPos : this.dunegonCoords)
		{
	...
	private void generatePositions()
    {
        ...
        while (lvt_2_1_.hasNext())
        {
            ...
            if (i < this.dunegonCoords.length)
            {
                this.dunegonCoords[i++] = new ChunkPos(structurestart.getChunkPosX(), structurestart.getChunkPosZ());
            }
        }
        ...
        if (l < this.dunegonCoords.length)
        {
            for (int i1 = 0; i1 < this.dunegonCoords.length; ++i1)
            {
                ...
                if (i1 >= l)
                {
                    this.dunegonCoords[i1] = new ChunkPos(j1, k1);
                }
                ...
                if (k == this.spread)
                {
                    ...
                    this.spread = Math.min(this.spread, this.dunegonCoords.length - i1);
